...
struct source_info;
struct list16;
/* Dynamic virtual channel callbacks exposed to backend modules. The xrdp
 * core owns channel framing and routes callbacks back to the module which
 * opened the channel. */
...
struct xrdp_mod
{
    ...
    int (*mod_drdynvc_ready)(struct xrdp_mod *v);
    ...
    int (*server_drdynvc_data_first)(struct xrdp_mod *v, int chan_id,
                                     const char *data, int data_bytes,
                                     int total_data_bytes);
    int (*server_drdynvc_data)(struct xrdp_mod *v, int chan_id,
                               const char *data, int data_bytes);
...
#define XRDP_DISPLAY_RESIZE_STATE_TO_STR(status) \
    ((status) == WMRZ_ENCODER_DELETE ? "WMRZ_ENCODER_DELETE" : \
     (status) == WMRZ_EGFX_RESET_GRAPHICS ? "WMRZ_EGFX_RESET_GRAPHICS" : \
